...
function showScreen(id) {
  ['homeScreen','loadingScreen','gameScreen','endScreen','historyScreen','gameMenuScreen','difficultyScreen','holScreen','holDiffScreen','holEndScreen','tournamentSizeScreen','tournamentScreen','tournamentEndScreen','tournamentDrawScreen','top10Screen','top10EndScreen'].forEach(s => {
...
async function startTop10() {
  ...
  document.getElementById('top10Input').placeholder = t('t10placeholder') || 'Type a movie title...';

  // Reset mode buttons
  document.getElementById('top10ModeRelax').classList.add('active');
  document.getElementById('top10ModeTimed').classList.remove('active');
  document.getElementById('top10TimerWrap').style.display = 'none';
  ...
  inp.addEventListener('input', function() { top10ShowSuggestions(this.value); });
  inp.addEventListener('blur', function() { setTimeout(top10HideSuggestions, 150); });
  inp.focus();
}
...
function top10ShowSuggestions(val) {
  ...
  if (!val || val.length < 1 || !top10Current) return;
  const norm = top10Normalize(val);
  // Build pool: all answers + some decoys from common movies not yet found
  const allAnswers = top10Current.answers;
  const matches = allAnswers.filter(a =>
    top10Normalize(a.name).includes(norm) &&
  ...
  matches.slice(0, 6).forEach(m => {
    ...
    const idx = top10Normalize(m.name).indexOf(norm);
    const original = m.name;
    // Bold the matching part
    const before = original.substring(0, idx);
    const match = original.substring(idx, idx + val.length);
    const after = original.substring(idx + val.length);
    item.innerHTML = `<span class="top10SuggGenre">${m.genre}</span> ${before}<strong>${match}</strong>${after}`;
    item.addEventListener('mousedown', function() {
      document.getElementById('top10Input').value = m.name;
      top10HideSuggestions();
      top10Submit();
    });
