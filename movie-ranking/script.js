...
function runBracketDraw(onDone) {
  ...
  const CARD_W    = 42, CARD_H = 60, V_GAP = 8, M_GAP = 10;
  const CHAMP_W   = 76, CHAMP_H = 108;
  const PAD_V     = 32;
  const COL_GAP   = 16;

  const totalR     = totalRounds();
  // Each side: round0 has tSize/2 matches, split left/right equally
  const leftCount0 = tSize / 2;   // total round-0 matches; ALL shown on left for animation
  // But for display: left side = first half of round0 matches, right = second half
  const sideCount  = tSize / 4;   // matches per side in round 0

  const matchBlockH = CARD_H * 2 + V_GAP;
  const sideH       = sideCount * (matchBlockH + M_GAP) - M_GAP;
  const H            = sideH + PAD_V * 2;

  // Width = (totalR columns) * (CARD_W + COL_GAP) per side, x2, + champion
  const colW   = CARD_W + COL_GAP;
  const sideW  = totalR * colW;
  const totalW = sideW * 2 + CHAMP_W + 40;
  ...
  function drawImg(img, x, y, w, h) {
    if (!img) { rr(x,y,w,h,3,'#2c3440',null); return; }
    ctx.save(); rr(x,y,w,h,3,null,null); ctx.clip();
    ctx.drawImage(img, x, y, w, h); ctx.restore();
  }

  // Position: slot x per side & round
  function slotX(side, r) {
    if (side === 'left')  return 8 + r * colW;
    else                   return totalW - 8 - CARD_W - r * colW;
  }
  // Position: match top-Y for round r, matchIdx mi (within that side, that round)
  function slotY(r, mi, count0) {
    const n    = count0 / Math.pow(2, r);
    const slotH = sideH / n;
  ...
  const imgCache = {};
  let loaded = 0;
  if (!tPool.length) { onDone(); return; }
  tPool.forEach(m => {
    const img = new Image(); img.crossOrigin = 'anonymous';
    img.onload  = () => { imgCache[m.titulo] = img; if (++loaded === tPool.length) startAnim(); };
    img.onerror = () => {                             if (++loaded === tPool.length) startAnim(); };
    img.src = m.imagen;
  });
  ...
  function startAnim() {
    ...
    // Build reveal list — all slots from both sides, shuffled
    const slots = [];
    ...
    for (let mi = 0; mi < half; mi++) {
      slots.push({ side:'right', mi, slot:'a', movie: round0[half + mi].a });
      slots.push({ side:'right', mi, slot:'b', movie: round0[half + mi].b });
    }
    ...
    let idx = 0;
    const delay = Math.max(35, 1400 / slots.length);

    function drawFrame() {
      clearCanvas();
      ['left','right'].forEach(side => {
        for (let mi = 0; mi < half; mi++) {
          const x  = slotX(side, 0);
          const y  = slotY(0, mi, half);
          const yb = y + CARD_H + V_GAP;
          const ka = `${side}_${mi}_a`, kb = `${side}_${mi}_b`;
          const m  = round0[side === 'left' ? mi : half + mi];
      ...
      ctx.fillStyle='#3d4e5f'; ctx.font='18px sans-serif'; ctx.textAlign='center';
      ctx.fillText('🏆', totalW/2, cy + CHAMP_H/2 + 6);
    }

    function revealNext() {
      if (idx >= slots.length) { setTimeout(onDone, 500); return; }
      const s = slots[idx++];
...
function tournamentPick(side) {
  ...
  // FIX 1: Only highlight the WINNING card border. No overlay (tilde) on cards.
  // The VS becomes the single checkmark.
...
/* ── SIDE BRACKET SVG ──
   FIX 2: Use half = round0.length / 2 (exact integer, no Math.ceil).
   FIX 3: For large tournaments (32), make the SVG taller to fit all matches.
   Left side  = matches 0 .. half-1  of each round (sliced proportionally).
   Right side = matches half .. end.
   The SVG height scales with sideCount (= tSize/4) regardless.
*/
...
function buildSVGBracket(side) {
  const CARD_W = 44, CARD_H = 60, V_GAP = 5, M_GAP = 8;
  const R_W    = 56;   // px per round column
  const PAD_T  = 10;

  const totalR     = totalRounds();
  const round0     = tBracket[0];
  const half       = round0.length / 2;   // exact: always even
  const sideCount  = half;                 // matches per side in round 0
  ...
  function matchTopY(roundIdx, matchIdx) {
    // In round r, there are sideCount/2^r matches on this side
    const n    = sideCount / Math.pow(2, roundIdx);
  ...
  for (let r = 0; r < Math.min(tBracket.length, totalR); r++) {
    ...
    // Slice this side's matches
    const sideRound = side === 'left'
      ? fullRound.slice(0, fullRound.length / 2)
      : fullRound.slice(fullRound.length / 2);

    // X position: left goes left→right (round 0 outermost), right goes right→left
    const x = side === 'left'
      ? r * R_W + 4
      : svgW - (r + 1) * R_W + 4;

    // Round label
    svg += `<text x="${x + CARD_W/2}" y="${PAD_T - 2}" text-anchor="middle" font-family="DM Sans,sans-serif" font-size="8" font-weight="bold" fill="${r === tRoundIndex ? '#00c030' : '#3d4e5f'}" letter-spacing="1">${getRoundName(r).toUpperCase().slice(0,7)}</text>`;
    ...
    sideRound.forEach((match, mi) => {
      ...
      const isCur = r === tRoundIndex && (() => {
        const fullIdx = side === 'left' ? mi : fullRound.length/2 + mi;
        return fullIdx === tMatchIndex;
      })();

      const uid = `${side}_r${r}_m${mi}`;

      // Card A
      svg += `<g opacity="${match.winner && !aWon ? 0.3 : 1}">`;
      ...
      // Card B
      svg += `<g opacity="${match.winner && !bWon ? 0.3 : 1}">`;
      ...
      // Connector to next round (only if winner chosen)
      if (match.winner && r + 1 < tBracket.length) {
        const nextRound    = tBracket[r + 1];
        const nextSide     = side === 'left'
          ? nextRound.slice(0, nextRound.length / 2)
          : nextRound.slice(nextRound.length / 2);
        const nextMi       = Math.floor(mi / 2);
        if (nextMi < nextSide.length) {
          const nextY      = matchTopY(r + 1, nextMi);
...
/* ── END SCREEN CANVAS ──
   FIX 2 (canvas): same half = round0.length / 2 (no Math.ceil).
   Bracket is symmetric left/right.
*/
...
async function drawBracketCanvas(champion) {
  ...
  const SCALE    = 2;
  const CARD_W   = 40, CARD_H = 56, V_GAP = 6, M_GAP = 10;
  const R_W      = 58;
  const PAD_X    = 28, PAD_Y = 28;
  const CHAMP_W  = 86, CHAMP_H = 122;
  const LINE     = '#00c030';

  const totalR   = totalRounds();
  const round0   = tBracket[0];
  const half     = round0.length / 2;   // exact
  const sideCount = half;               // matches per side in round 0

  const matchBlockH = CARD_H * 2 + V_GAP;
  const sideH       = sideCount * (matchBlockH + M_GAP) - M_GAP;
  const H            = sideH + PAD_Y * 2;
  const W            = PAD_X * 2 + totalR * R_W * 2 + CHAMP_W + 20;
  ...
  function drawImg(img,x,y,w,h) {
    ctx.save(); clip(x,y,w,h,3); ctx.clip();
    if (img) ctx.drawImage(img,x,y,w,h);
    else { ctx.fillStyle='#2c3440'; ctx.fill(); }
    ctx.restore();
  }
  function strokeBox(x,y,w,h,clr,lw) {
    clip(x,y,w,h,3); ctx.strokeStyle=clr; ctx.lineWidth=lw; ctx.stroke();
  }

  function matchTopY(r, mi) {
    const n = sideCount / Math.pow(2, r);
    const slotH = sideH / n;
  ...
  function drawSide(isLeft) {
    for (let r = 0; r < tBracket.length; r++) {
      const full   = tBracket[r];
      const side   = isLeft ? full.slice(0, full.length/2) : full.slice(full.length/2);
      ...
      side.forEach((match, mi) => {
        ...
        // Connector
        if (match.winner && r+1 < tBracket.length) {
          const nextFull = tBracket[r+1];
          const nextSide = isLeft ? nextFull.slice(0,nextFull.length/2) : nextFull.slice(nextFull.length/2);
          const nmi      = Math.floor(mi/2);
          if (nmi < nextSide.length) {
  ...
  ctx.shadowColor = '#00c030'; ctx.shadowBlur = 20;
  drawImg(imgs[champion.titulo], cx, cy, CHAMP_W, CHAMP_H);
  ctx.shadowBlur = 0;
  strokeBox(cx, cy, CHAMP_W, CHAMP_H, LINE, 2.5);
  ctx.font = '14px sans-serif'; ctx.textAlign = 'center';
  ctx.fillText('🏆', W/2, cy-6);
  ctx.fillStyle='#fff'; ctx.font='bold 8px DM Sans,sans-serif';
  ctx.fillText(champion.titulo.slice(0,22), W/2, cy+CHAMP_H+13);
