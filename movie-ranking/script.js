...
const LANG = {
  es: {
    ...
    // Tournament
    game3name: 'Torneo',
    game3desc: 'Bracket de 8 o 16 películas. ¿Cuál es tu favorita?',
    tournamentSize: 'Tamaño del torneo',
    tournament8name: 'Cuartos',
    tournament8desc: '3 rondas — rápido y decisivo',
    tournament16name: 'Épico',
    tournament16desc: '4 rondas — el torneo completo',
    tournamentChampion: '¡Campeón!',
    tournamentRound: (r, total) => `Ronda ${r} de ${total}`,
    tournamentMatch: (m, total) => `Partido ${m} de ${total}`,
    roundNames: ['Octavos', 'Cuartos', 'Semifinal', 'Final'],
  ...
  en: {
    ...
    game3desc: 'Bracket of 8 or 16 films. Which one is your champion?',
    tournamentSize: 'Tournament size',
    tournament8name: 'Quick',
    tournament8desc: '3 rounds — fast and decisive',
    tournament16name: 'Epic',
    tournament16desc: '4 rounds — the full bracket',
...
let tRoundIndex = 0;     // current round (0-based)
let tMatchIndex = 0;     // current match within round
let tSize = 8;           // 8 or 16
let tPicking = false;    // prevent double-click
...
function totalRounds() {
  return Math.log2(tSize); // 8→3, 16→4
}

function getRoundName(roundIdx) {
  const names = LANG[currentLang].roundNames;
  const total = totalRounds();
  // Map from round index to name
  // roundIdx 0 = earliest, totalRounds-1 = Final
  const fromEnd = total - 1 - roundIdx; // 0=Final, 1=Semi, 2=Quarters, 3=R16
  if (fromEnd === 0) return names[3] || 'Final';
  if (fromEnd === 1) return names[2] || 'Semifinals';
  if (fromEnd === 2) return names[1] || 'Quarterfinals';
  return names[0] || 'Round of 16';
}

function renderTournamentMatch() {
  const round = tBracket[tRoundIndex];
  const match = round[tMatchIndex];
  const roundName = getRoundName(tRoundIndex);
  const matchesInRound = round.length;

  document.getElementById('tournamentRoundLabel').textContent = roundName;
  document.getElementById('tournamentMatchLabel').textContent =
    t('tournamentMatch', tMatchIndex + 1, matchesInRound);

  // Card A
  document.getElementById('tPosterA').src = match.a.imagen;
  document.getElementById('tTitleA').textContent = match.a.titulo;
  document.getElementById('tRatingA').textContent = match.a.rating ? match.a.rating + ' ★' : '';

  // Card B
  document.getElementById('tPosterB').src = match.b.imagen;
  document.getElementById('tTitleB').textContent = match.b.titulo;
  document.getElementById('tRatingB').textContent = match.b.rating ? match.b.rating + ' ★' : '';

  // Reset overlays and VS
...
function renderBracket() {
  const wrap = document.getElementById('tournamentBracketWrap');
  if (!wrap) return;

  const totalR = totalRounds();
  // Build a compact visual bracket
  let html = '<div class="tBracket">';

  for (let r = 0; r < tBracket.length; r++) {
    const round = tBracket[r];
    const isCurrentRound = r === tRoundIndex;
    html += `<div class="tBracketRound">`;
    html += `<div class="tBracketRoundName" style="color:${isCurrentRound ? '#00c030' : '#677988'}">${getRoundName(r)}</div>`;
    round.forEach((match, mi) => {
      const aCurrent = isCurrentRound && mi === tMatchIndex && match.winner === null;
      const bCurrent = aCurrent;
      const aWon = match.winner && match.winner.titulo === match.a.titulo;
      const bWon = match.winner && match.winner.titulo === match.b.titulo;

      html += `<div class="tBracketMatch ${isCurrentRound && mi === tMatchIndex ? 'tBracketMatchActive' : ''}">
        <div class="tBracketSlot ${aWon ? 'tBracketWinner' : ''} ${match.winner && !aWon ? 'tBracketLoser' : ''}">
          ${match.a.titulo.length > 18 ? match.a.titulo.slice(0,16)+'…' : match.a.titulo}
        </div>
        <div class="tBracketSlot ${bWon ? 'tBracketWinner' : ''} ${match.winner && !bWon ? 'tBracketLoser' : ''}">
          ${match.b.titulo.length > 18 ? match.b.titulo.slice(0,16)+'…' : match.b.titulo}
        </div>
      </div>`;
    });
    html += `</div>`;
