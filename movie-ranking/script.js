...
function showScreen(id) {
  ['homeScreen','loadingScreen','gameScreen','endScreen','historyScreen','gameMenuScreen','difficultyScreen','holScreen','holDiffScreen','holEndScreen','tournamentSizeScreen','tournamentScreen','tournamentEndScreen'].forEach(s => {
...
function startTournamentGame(size) {
  if (!peliculasOriginal.length) { showHome(); return; }
  tSize = size;
  const pool = shuffle(peliculasOriginal).slice(0, size);
  const firstRound = [];
  for (let i = 0; i < pool.length; i += 2)
    firstRound.push({ a: pool[i], b: pool[i+1], winner: null });
  tBracket = [firstRound];
  tRoundIndex = 0;
  tMatchIndex = 0;
  tPicking = false;
...
function renderTournamentMatch() {
  ...
  const skipBtn = document.getElementById('tSkipBtn');
  if (skipBtn) skipBtn.style.display = isPopularMode ? 'inline-block' : 'none';
}

function tournamentSkip() {
  const round = tBracket[tRoundIndex];
  const match = round[tMatchIndex];
  ...
  const picks = shuffle(available);
  if (picks.length >= 2) { match.a = picks[0]; match.b = picks[1]; }
  else if (picks.length === 1) { match.b = picks[0]; }
  renderTournamentMatch();
  renderTBrackets();
}
...
function renderTBrackets() {
  ...
  const firstRound = tBracket[0];
  const half = Math.ceil(firstRound.length / 2);
  const leftMatches = firstRound.slice(0, half);
  const rightMatches = firstRound.slice(half);
  leftEl.innerHTML = buildSidePanel(leftMatches, 0, 'left');
  rightEl.innerHTML = buildSidePanel(rightMatches, half, 'right');
}

function buildSidePanel(matches, offset, side) {
  let html = '<div class="tSidePanel">';
  matches.forEach((match, i) => {
    const globalIdx = offset + i;
    const isCurrent = tRoundIndex === 0 && globalIdx === tMatchIndex;
    const aWon = match.winner && match.winner.titulo === match.a.titulo;
    const bWon = match.winner && match.winner.titulo === match.b.titulo;
    html += `<div class="tSideMatch ${isCurrent ? 'tSideActive' : ''} ${match.winner ? 'tSideDone' : ''}">
      <div class="tSideSlot ${aWon?'tSideWon':''} ${match.winner&&!aWon?'tSideLost':''}">
        <img class="tSideThumb" src="${match.a.imagen}" title="${match.a.titulo}">
      </div>
      <div class="tSideDivider"></div>
      <div class="tSideSlot ${bWon?'tSideWon':''} ${match.winner&&!bWon?'tSideLost':''}">
        <img class="tSideThumb" src="${match.b.imagen}" title="${match.b.titulo}">
      </div>
    </div>`;
  });
  html += '</div>';
...
function showTournamentEnd(champion) {
  ...
  applyLang();
  setTimeout(() => drawBracketCanvas(champion), 100);
}
...
async function drawBracketCanvas(champion) {
  ...
  const SCALE = 2;
  const CARD_W = 52, CARD_H = 72, CARD_R = 4;
  const MATCH_GAP = 12;  // vertical gap between the two slots of a match
  const ROUND_W = 72;    // horizontal spacing per round column
  const SIDE_PAD = 16;
  const TOP_PAD = 40;
  const CHAMPION_W = 110, CHAMPION_H = 155;

  const totalR = totalRounds(); // rounds per side
  // Total rounds in bracket = totalR (left side contributes totalR rounds)
  // Layout: left side rounds go right-to-left, right side rounds go left-to-right
  // Center = champion card

  // Calculate canvas dimensions
  const sideWidth = totalR * ROUND_W + SIDE_PAD;
  const W = sideWidth * 2 + CHAMPION_W + 40;

  // Height: first round has tSize/2 matches, spaced out
  const matchesR0 = tSize / 2;
  const matchH = CARD_H * 2 + MATCH_GAP;
  const matchSpacingR0 = matchH + 20;
  const H = TOP_PAD + matchesR0 * matchSpacingR0 + 60;
  ...
  const allMovies = [];
  tBracket.forEach(round => round.forEach(m => { allMovies.push(m.a); allMovies.push(m.b); }));
  const uniqueMovies = [...new Map(allMovies.map(m => [m.titulo, m])).values()];
  const imgCache = {};
  await Promise.all(uniqueMovies.map(m => new Promise(resolve => {
    const img = new Image();
  ...
  // Helper: draw rounded rect clip + image
  function drawPoster(img, x, y, w, h) {
    ctx.save();
    ctx.beginPath();
    const r = CARD_R;
  ...
  function drawMatchBox(match, x, y, won_a, won_b) {
    const imgA = imgCache[match.a.titulo];
    const imgB = imgCache[match.b.titulo];
    // Card A
    ctx.globalAlpha = won_b ? 0.35 : 1;
    drawPoster(imgA, x, y, CARD_W, CARD_H);
    if (won_a) {
      ctx.globalAlpha = 1;
      ctx.strokeStyle = '#00c030'; ctx.lineWidth = 2;
      ctx.beginPath();
      const r=CARD_R, bx=x, by=y, bw=CARD_W, bh=CARD_H;
    ...
    // Card B
    ctx.globalAlpha = won_a ? 0.35 : 1;
    drawPoster(imgB, x, y + CARD_H + MATCH_GAP, CARD_W, CARD_H);
    if (won_b) {
      ctx.globalAlpha = 1;
      ctx.strokeStyle = '#00c030'; ctx.lineWidth = 2;
      const by2 = y + CARD_H + MATCH_GAP;
      ctx.beginPath();
      const r=CARD_R, bx=x, bw=CARD_W, bh=CARD_H;
      ctx.moveTo(bx+r,by2); ctx.lineTo(bx+bw-r,by2); ctx.arcTo(bx+bw,by2,bx+bw,by2+r,r);
      ctx.lineTo(bx+bw,by2+bh-r); ctx.arcTo(bx+bw,by2+bh,bx+bw-r,by2+bh,r);
      ctx.lineTo(bx+r,by2+bh); ctx.arcTo(bx,by2+bh,bx,by2+bh-r,r);
      ctx.lineTo(bx,by2+r); ctx.arcTo(bx,by2,bx+r,by2,r);
      ctx.closePath(); ctx.stroke();
    }
    ctx.globalAlpha = 1;
  }

  // Position calculation: for each round and match, get center-Y
  // Round 0 has tSize/2 matches; each subsequent round halves
  function getMatchY(roundIdx, matchIdx) {
    const matchesInRound = tSize / Math.pow(2, roundIdx + 1);
    const totalH_forRound = H - TOP_PAD - 40;
    const spacing = totalH_forRound / matchesInRound;
    return TOP_PAD + spacing * (matchIdx + 0.5) - (CARD_H * 2 + MATCH_GAP) / 2;
  }

  // Draw left side: rounds go left (round 0) to center (final)
  // round 0 at x = SIDE_PAD, round 1 at SIDE_PAD+ROUND_W, ...
  // last left round at x = sideWidth - ROUND_W
  for (let r = 0; r < tBracket.length - 1 || (tBracket.length > 0 && r < totalR); r++) {
    if (r >= tBracket.length) break;
    const round = tBracket[r];
    const half = Math.ceil(round.length / 2);
    const leftMatches = round.slice(0, half);
    const x = SIDE_PAD + r * ROUND_W;

    // Round label
    ctx.fillStyle = r === tRoundIndex ? '#00c030' : '#3d4e5f';
    ctx.font = `bold ${9}px DM Sans, sans-serif`;
    ctx.textAlign = 'center';
    ctx.fillText(getRoundName(r).toUpperCase(), x + CARD_W/2, TOP_PAD - 10);

    leftMatches.forEach((match, mi) => {
      const aWon = match.winner && match.winner.titulo === match.a.titulo;
      const bWon = match.winner && match.winner.titulo === match.b.titulo;
      const y = getMatchY(r, mi);
      drawMatchBox(match, x, y, aWon, bWon);

      // Connector line to next round
      if (match.winner && r + 1 < tBracket.length) {
        const nextRound = tBracket[r+1];
        const nextMatchIdx = Math.floor(mi / 2);
        if (nextMatchIdx < Math.ceil(nextRound.length / 2)) {
          const midY = y + (match.winner.titulo === match.a.titulo ? CARD_H/2 : CARD_H + MATCH_GAP + CARD_H/2);
          const nextX = SIDE_PAD + (r+1) * ROUND_W;
          const nextY = getMatchY(r+1, nextMatchIdx);
          const nextMidY = nextY + CARD_H + MATCH_GAP/2;
          ctx.strokeStyle = '#00c030';
          ctx.lineWidth = 1.5;
          ctx.globalAlpha = 0.5;
          ctx.beginPath();
          ctx.moveTo(x + CARD_W, midY);
          ctx.lineTo(x + CARD_W + (nextX - x - CARD_W)/2, midY);
          ctx.lineTo(x + CARD_W + (nextX - x - CARD_W)/2, nextMidY);
          ctx.lineTo(nextX, nextMidY);
          ctx.stroke();
          ctx.globalAlpha = 1;
        }
      }
  ...
  // Draw right side: rounds go right (round 0) to center
  // Round 0 at x = W - SIDE_PAD - CARD_W, going left
  for (let r = 0; r < tBracket.length - 1 || (tBracket.length > 0 && r < totalR); r++) {
    if (r >= tBracket.length) break;
    const round = tBracket[r];
    const half = Math.ceil(round.length / 2);
    const rightMatches = round.slice(half);
    const x = W - SIDE_PAD - CARD_W - r * ROUND_W;
    ...
    rightMatches.forEach((match, mi) => {
      const aWon = match.winner && match.winner.titulo === match.a.titulo;
      const bWon = match.winner && match.winner.titulo === match.b.titulo;
      const globalMi = half + mi;
      const y = getMatchY(r, globalMi - (round.length - rightMatches.length));
      drawMatchBox(match, x, y, aWon, bWon);

      if (match.winner && r + 1 < tBracket.length) {
        const nextRound = tBracket[r+1];
        const nextMatchIdx = Math.floor(mi / 2) + Math.ceil(nextRound.length / 2);
        if (nextMatchIdx < nextRound.length) {
          const midY = y + (match.winner.titulo === match.a.titulo ? CARD_H/2 : CARD_H + MATCH_GAP + CARD_H/2);
          const nextX = W - SIDE_PAD - CARD_W - (r+1) * ROUND_W;
          const nextY = getMatchY(r+1, nextMatchIdx - Math.ceil(tBracket[r+1].length/2));
          const nextMidY = nextY + CARD_H + MATCH_GAP/2;
          ctx.strokeStyle = '#00c030';
          ctx.lineWidth = 1.5;
          ctx.globalAlpha = 0.5;
          ctx.beginPath();
          ctx.moveTo(x, midY);
          ctx.lineTo(x - (x - nextX - CARD_W)/2, midY);
          ctx.lineTo(x - (x - nextX - CARD_W)/2, nextMidY);
          ctx.lineTo(nextX + CARD_W, nextMidY);
          ctx.stroke();
          ctx.globalAlpha = 1;
        }
      }
  ...
  // Draw champion in center
  const champImg = imgCache[champion.titulo];
  const cx = W/2 - CHAMPION_W/2;
  const cy = H/2 - CHAMPION_H/2;
  // Glow
  ctx.shadowColor = '#00c030';
  ctx.shadowBlur = 30;
  drawPoster(champImg, cx, cy, CHAMPION_W, CHAMPION_H);
  ctx.shadowBlur = 0;
  // Border
  ctx.strokeStyle = '#00c030'; ctx.lineWidth = 3;
  const r2=6, bx2=cx, by2=cy, bw2=CHAMPION_W, bh2=CHAMPION_H;
  ctx.beginPath();
  ctx.moveTo(bx2+r2,by2); ctx.lineTo(bx2+bw2-r2,by2); ctx.arcTo(bx2+bw2,by2,bx2+bw2,by2+r2,r2);
  ctx.lineTo(bx2+bw2,by2+bh2-r2); ctx.arcTo(bx2+bw2,by2+bh2,bx2+bw2-r2,by2+bh2,r2);
  ctx.lineTo(bx2+r2,by2+bh2); ctx.arcTo(bx2,by2+bh2,bx2,by2+bh2-r2,r2);
  ctx.lineTo(bx2,by2+r2); ctx.arcTo(bx2,by2,bx2+r2,by2,r2);
  ctx.closePath(); ctx.stroke();
  // Trophy + name
  ctx.textAlign = 'center';
  ...
  const champTitle = champion.titulo.length > 20 ? champion.titulo.slice(0,18)+'…' : champion.titulo;
  ctx.fillText(champTitle, W/2, cy + CHAMPION_H + 16);

  // Logo bottom
  ctx.fillStyle = '#677988';
  ctx.font = '500 10px DM Sans, sans-serif';
  ctx.textAlign = 'center';
  ctx.fillText('RANKBOXD.VERCEL.APP', W/2, H - 10);
}
