...
async function shareRanking() {
  ...
  let starsHtml = '';
  for (let i = 1; i <= 5; i++) {
    if (i <= Math.floor(selectedStars)) starsHtml += '<span style="color:#00c030;font-size:18px;">★</span>';
    else if (i === Math.ceil(selectedStars) && selectedStars % 1 !== 0) starsHtml += '<span style="color:#00c030;font-size:18px;">½</span>';
    else starsHtml += '<span style="color:#2c3440;font-size:18px;">★</span>';
  }
  ...
  try {
    const canvas = await html2canvas(document.querySelector('.shareCardInner'), {
      backgroundColor: '#14181c', scale: 2, useCORS: true, allowTaint: false, logging: false,
    });
