...
/* ── HIGHER OR LOWER ── */
let holMovies = [];
let holCurrent = null; // current "champion"
let holNext = null;
let holStreak = 0;
let holBest = 0;
let holMode = 'normal'; // 'normal' or 'hard'
let holChampionWins = 0;
let holUsed = [];
...
function startHOLGame(mode) {
  ...
  if (holMovies.length < 2) {
    alert('Necesitás al menos 2 películas con rating.');
    return;
  }
  ...
  // Show/hide tie button
  document.getElementById('holTieBtn').style.display = holMode === 'hard' ? 'block' : 'none';

  updateHolStreak();
  holPickNext();
  holPickNext(); // pick two
}

function holPickNext() {
  const available = holMovies.filter(m => !holUsed.includes(m.titulo));
  if (available.length === 0) holUsed = []; // reset if exhausted
  ...
  if (!holCurrent) {
    holCurrent = movie;
    setHolCard('A', holCurrent, true);
  } else {
    holNext = movie;
    setHolCard('B', holNext, false);
  }
}

function setHolCard(side, movie, showRating) {
  const poster = document.getElementById(`holPoster${side}`);
  const title = document.getElementById(`holTitle${side}`);
  const rating = document.getElementById(`holRating${side}`);
  const card = document.getElementById(`holCard${side}`);
  ...
  if (showRating) {
    ...
    rating.style.color = '#00c030';
    card.classList.add('holCardRevealed');
  } else {
    rating.innerHTML = `<span data-i18n="holQuestion">${t('holQuestion')}</span>`;
    rating.style.color = '#677988';
    card.classList.remove('holCardRevealed');
  }
...
function holChoose(choice) {
  if (!holCurrent || !holNext) return;

  const ratingA = holCurrent.rating;
  const ratingB = holNext.rating;
  const isTie = Math.abs(ratingA - ratingB) < 0.05;

  let correct = false;

  if (choice === 'A') correct = ratingA > ratingB || (isTie && holMode === 'normal');
  else if (choice === 'B') correct = ratingB > ratingA || (isTie && holMode === 'normal');
  else if (choice === 'tie') correct = isTie;

  // Reveal B rating
  const ratingEl = document.getElementById('holRatingB');
  ratingEl.textContent = `${ratingB} ★`;
  ratingEl.style.color = '#00c030';
  ...
  if (correct) {
    ...
    // After delay, next round
    setTimeout(() => {
      holChampionWins++;
      if (holChampionWins >= 2) {
        // Force new champion after 2 wins
        holCurrent = null;
        holChampionWins = 0;
        holPickNext();
        holPickNext();
      } else {
        holCurrent = ratingA >= ratingB ? holCurrent : holNext;
        holNext = null;
        setHolCard('A', holCurrent, true);
        holPickNext();
      }
