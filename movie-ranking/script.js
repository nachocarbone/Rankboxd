...
function showScreen(id) {
  ...
  if (totEl) totEl.textContent = window.TOTAL_DYNAMIC || TOTAL;
...
function startGameWithDifficulty(total) {
  window.TOTAL_DYNAMIC = total;
  startGame();
}
...
function startGame() {
  ...
  const total = window.TOTAL_DYNAMIC || TOTAL;
  peliculas = shuffle(peliculasOriginal).slice(0, total);
  ranking = new Array(window.TOTAL_DYNAMIC || TOTAL).fill(null);
...
function updateCounter() {
  const tot = window.TOTAL_DYNAMIC || TOTAL;
  document.getElementById('countCurrent').textContent = Math.min(current + 1, tot);
}
