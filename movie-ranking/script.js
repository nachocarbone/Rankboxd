...
const LANG = {
  es: {
    ...
    game4name: 'Top 10',
    game4desc: 'Adiviná las 10 películas del día. Nuevo desafío cada día.',
    t10relax: 'Relax',
    t10relaxDesc: 'Sin límite de tiempo',
    t10timedDesc: 'Contrarreloj',
    t10timed: '⏱ 90s',
    t10calendar: 'Tu racha',
    t10share: 'Compartir resultado',
    t10placeholder: 'Escribí un título...',
  ...
  en: {
    ...
    game4name: 'Top 10',
    game4desc: 'Guess the 10 films of the day. New challenge every day.',
    t10relax: 'Relax',
    t10relaxDesc: 'Sin límite de tiempo',
    t10timedDesc: 'Contrarreloj',
    t10timed: '⏱ 90s',
    t10calendar: 'Your streak',
    t10share: 'Share result',
    t10placeholder: 'Type a movie title...',
...
function applyLang() {
  ...
  // Update top10 question if active
  if (top10Current) {
    const qEl = document.getElementById('top10Question');
    if (qEl) qEl.textContent = currentLang === 'es' ? top10Current.question_es : top10Current.question;
  }
}
...
function showScreen(id) {
  ['homeScreen','loadingScreen','gameScreen','endScreen','historyScreen','gameMenuScreen','difficultyScreen','holScreen','holDiffScreen','holEndScreen','tournamentSizeScreen','tournamentScreen','tournamentEndScreen','tournamentDrawScreen','top10Screen','top10EndScreen','top10ModeScreen','top10CalScreen'].forEach(s => {
...
let top10Mode = 'relax';
let top10Timer = null;
let top10TimeLeft = 90;
let top10Finished = false;

async function startTop10() {
  try {
    const res = await fetch('challenges.json');
    top10Challenges = await res.json();
  } catch(e) {
    top10Challenges = [];
  }
  if (!top10Challenges.length) { alert('No challenges available.'); return; }

  const now = new Date();
  const start = new Date(now.getFullYear(), 0, 0);
  const dayOfYear = Math.floor((now - start) / 86400000);
  const idx = dayOfYear % top10Challenges.length;
  top10Current = top10Challenges[idx];

  // Show mode selection screen
  document.getElementById('top10ModeDay').textContent = dayOfYear;
  document.getElementById('top10ModeQuestion').textContent = currentLang === 'es' ? top10Current.question_es : top10Current.question;
  showScreen('top10ModeScreen');
  applyLang();
}

function top10StartGame(mode) {
  top10Found = [];
  top10Finished = false;
  top10Mode = mode;
  ...
  document.getElementById('top10DayNum').textContent = dayOfYear;
  document.getElementById('top10Question').textContent = currentLang === 'es' ? top10Current.question_es : top10Current.question;

  // Timer setup
  clearInterval(top10Timer);
  const timerWrap = document.getElementById('top10TimerWrap');
  if (mode === 'timed') {
    timerWrap.style.display = 'flex';
    top10TimeLeft = 90;
    top10UpdateTimerDisplay();
    top10Timer = setInterval(() => {
      top10TimeLeft--;
      top10UpdateTimerDisplay();
      if (top10TimeLeft <= 0) { clearInterval(top10Timer); top10EndGame(false); }
    }, 1000);
  } else {
  ...
  top10RenderList();
  showScreen('top10Screen');
  applyLang();

  const inp = document.getElementById('top10Input');
  inp.value = '';
  // Remove old listeners by cloning
  const newInp = inp.cloneNode(true);
  inp.parentNode.replaceChild(newInp, inp);
  newInp.addEventListener('keydown', function(e) {
    if (e.key === 'Enter') { top10HideSuggestions(); top10Submit(); }
    if (e.key === 'Escape') top10HideSuggestions();
  });
  newInp.addEventListener('input', function() { top10ShowSuggestions(this.value); });
  newInp.addEventListener('blur', function() { setTimeout(top10HideSuggestions, 150); });
  newInp.focus();
}

function top10SetMode(mode) {
  top10Mode = mode;
  clearInterval(top10Timer);
  document.getElementById('top10ModeRelax').classList.toggle('active', mode === 'relax');
  document.getElementById('top10ModeTimed').classList.toggle('active', mode === 'timed');
  const timerWrap = document.getElementById('top10TimerWrap');
...
function top10UpdateTimerDisplay() {
  const m = Math.floor(top10TimeLeft / 60);
  const s = top10TimeLeft % 60;
  const el = document.getElementById('top10Timer');
  el.textContent = m + ':' + String(s).padStart(2, '0');
  el.style.color = top10TimeLeft <= 15 ? '#e63946' : '#00c030';
}

function top10Normalize(str) {
...
function top10Submit() {
  if (top10Finished) return;
  const input = document.getElementById('top10Input');
  const val = input.value.trim();
  if (!val) return;

  const normalized = top10Normalize(val);
  const feedback = document.getElementById('top10Feedback');

  // Check if already found
  const alreadyFound = top10Found.some(f => top10Normalize(f.name) === normalized);
  if (alreadyFound) {
    top10ShowFeedback('Already found!', '#677988');
    input.value = '';
    return;
  }

  // Check against answers
  const match = top10Current.answers.find(a => top10Normalize(a.name) === normalized);
  if (match) {
    top10Found.push(match);
    top10RenderList();
    top10ShowFeedback('✓ ' + match.name, '#00c030');
    input.value = '';
    if (top10Found.length === top10Current.answers.length) {
      clearInterval(top10Timer);
      setTimeout(() => top10EndGame(true), 600);
    }
  } else {
    top10ShowFeedback('✗ Not in the list', '#e63946');
    input.classList.add('top10InputShake');
    setTimeout(() => input.classList.remove('top10InputShake'), 400);
    input.value = '';
  }
}

function top10ShowFeedback(msg, color) {
  const el = document.getElementById('top10Feedback');
  el.textContent = msg;
...
function top10RenderList() {
  const list = document.getElementById('top10List');
  list.innerHTML = '';
  top10Current.answers.forEach((answer, i) => {
    const found = top10Found.find(f => f.name === answer.name);
    const row = document.createElement('div');
    row.className = 'top10Row' + (found ? ' top10RowFound' : '');
    row.innerHTML = `
      <span class="top10Num">${i + 1}</span>
      <span class="top10Genre">${answer.genre}</span>
      <span class="top10Name">${found ? answer.name : ''}</span>
    `;
    list.appendChild(row);
  });
}

function top10EndGame(completed) {
  top10Finished = true;
  clearInterval(top10Timer);

  // Save to localStorage
  const today = new Date().toISOString().split('T')[0];
  const history = JSON.parse(localStorage.getItem('top10History') || '{}');
  history[today] = { completed, found: top10Found.length, total: top10Current.answers.length };
  localStorage.setItem('top10History', JSON.stringify(history));

  // End screen
  const titleEl = document.getElementById('top10EndTitle');
  const scoreEl = document.getElementById('top10EndScore');
  titleEl.textContent = completed ? '🎉 COMPLETED!' : 'GAME OVER';
  titleEl.style.color = completed ? '#00c030' : '#e63946';
  scoreEl.textContent = top10Found.length + ' / ' + top10Current.answers.length + ' found';

  // Reveal missing answers
  const revealEl = document.getElementById('top10EndReveal');
  if (!completed) {
    const missing = top10Current.answers.filter(a => !top10Found.find(f => f.name === a.name));
    revealEl.innerHTML = '<div class="top10RevealTitle">You missed:</div>' +
      missing.map(m => `<div class="top10RevealItem"><span class="top10Genre">${m.genre}</span> ${m.name}</div>`).join('');
  } else {
    revealEl.innerHTML = '';
  }
  ...
  // Calendar
  top10RenderCalendar();
  showScreen('top10EndScreen');
}

function top10RenderCalendar() {
  const history = JSON.parse(localStorage.getItem('top10History') || '{}');
  const cal = document.getElementById('top10Calendar');
  const days = Object.keys(history).sort().slice(-30);
  cal.innerHTML = days.map(day => {
    const d = history[day];
    const color = d.completed ? '#00c030' : (d.found > 0 ? '#ffb400' : '#e63946');
    const label = day.slice(5); // MM-DD
    return `<div class="top10CalDay" style="background:${color}" title="${label}: ${d.found}/${d.total}"></div>`;
  }).join('');
}


function showTop10Calendar() {
  const history = JSON.parse(localStorage.getItem('top10History') || '{}');
  ...
  while (true) {
    ...
    if (history[key] && history[key].completed) { streak++; d.setDate(d.getDate()-1); }
    else break;
  }
  document.getElementById('top10StreakNum').textContent = streak + ' day streak 🔥';

  const cal = document.getElementById('top10CalGrid');
  cal.innerHTML = days.map(day => {
    const d = history[day];
    const color = d.completed ? '#00c030' : (d.found > 0 ? '#ffb400' : '#e63946');
    const label = day.slice(5);
    return `<div class="top10CalDay2" style="background:${color}" title="${label}: ${d.found}/${d.total}">
      <span class="top10CalDayLabel">${label}</span>
    </div>`;
  }).join('');
  showScreen('top10CalScreen');
}
...
function top10Share() {
  ...
  if (!d) return;
  const emoji = d.completed ? '🟩' : '🟥';
  const squares = Array(d.total).fill(0).map((_, i) => i < d.found ? '🟩' : '⬛').join('');
  const text = `Rankboxd Top 10 - Day ${document.getElementById('top10DayNum').textContent}\n${squares}\n${d.found}/${d.total} found\nrankboxd.vercel.app`;
  navigator.clipboard.writeText(text).then(() => alert('Copied to clipboard!'));
}


function top10ShowSuggestions(val) {
  top10HideSuggestions();
  if (!val || val.length < 3 || !top10Current) return;
  const norm = top10Normalize(val);
  const allAnswers = top10Current.answers;
  const matches = allAnswers.filter(a =>
    top10Normalize(a.name).startsWith(norm) &&
    !top10Found.find(f => f.name === a.name)
  );
  if (!matches.length) return;

  const wrap = document.getElementById('top10InputWrap');
  const list = document.createElement('div');
  list.id = 'top10Suggestions';
  list.className = 'top10Suggestions';

  matches.slice(0, 6).forEach(m => {
    const item = document.createElement('div');
    item.className = 'top10SuggItem';
    const normName = top10Normalize(m.name);
    const matchLen = val.length;
    const bold = m.name.substring(0, matchLen);
    const rest = m.name.substring(matchLen);
    item.innerHTML = `<span class="top10SuggGenre">${m.genre}</span><span><strong>${bold}</strong>${rest}</span>`;
...
function top10HideSuggestions() {
  const s = document.getElementById('top10Suggestions');
  if (s) s.remove();
}
