...
/* ── TOURNAMENT ── */
let tBracket = [];       // array of rounds; each round = array of {a, b, winner}
let tRoundIndex = 0;
let tMatchIndex = 0;
let tSize = 8;
let tPicking = false;
...
function totalRounds() {
  return Math.log2(tSize);
}

function getRoundName(roundIdx) {
  const names = LANG[currentLang].roundNames; // length 5: R32, R16, QF, SF, F
  const total = totalRounds(); // 3, 4, or 5
  // Map roundIdx to names array from the end
  // roundIdx = total-1 → Final (names[4])
  // roundIdx = total-2 → SF (names[3])
  // etc.
...
function renderTournamentMatch() {
  ...
  document.getElementById('tVS').style.color = '';
  ...
  // Show/hide skip button (popular mode only)
  const skipBtn = document.getElementById('tSkipBtn');
  if (skipBtn) skipBtn.style.display = isPopularMode ? 'inline-block' : 'none';
}

function fillTCard(side, movie) {
  document.getElementById('tPoster' + side).src = movie.imagen;
  document.getElementById('tTitle' + side).textContent = movie.titulo;
  const rEl = document.getElementById('tRating' + side);
  rEl.textContent = movie.rating ? movie.rating + ' ★' : '';
}

function tournamentSkip() {
  // Replace both current movies with unseen ones from the pool
  const round = tBracket[tRoundIndex];
  const match = round[tMatchIndex];
  const usedTitles = new Set(
    tBracket.flatMap(r => r.flatMap(m => [m.a.titulo, m.b.titulo]))
  );
...
function showTournamentEnd(champion) {
  ...
  document.getElementById('tournamentWinnerRating').textContent =
    champion.rating ? champion.rating + ' ★' : '';
  applyLang();
}

/* ── BRACKET SIDE PANELS ── */
function renderSideBrackets() {
  ...
  // Split rounds: left side shows first half of rounds, right shows second half
  // For 8 films: 3 rounds → left: round 0 (4 matches), right: round 1+2
  // For 16: 4 rounds → left: rounds 0-1, right: rounds 2-3
  // For 32: 5 rounds → left: rounds 0-1, right: rounds 3-4, round 2 is center info
  // Simpler: left panel = first round only (most slots), right panel = remaining rounds

  const firstRound = tBracket[0];
  const half = Math.floor(firstRound.length / 2);
  const leftMatches = firstRound.slice(0, half);
  const rightMatches = firstRound.slice(half);

  leftEl.innerHTML = buildBracketColumn(leftMatches, 0, 0, half, 'left');
  rightEl.innerHTML = buildBracketColumn(rightMatches, 0, half, firstRound.length, 'right');
}

function buildBracketColumn(matches, roundIdx, startIdx, endIdx, side) {
  let html = `<div class="tSideBracket">`;
  html += `<div class="tSideRoundName" style="color:${roundIdx === tRoundIndex ? '#00c030':'#677988'}">${getRoundName(roundIdx)}</div>`;

  matches.forEach((match, i) => {
    const globalIdx = startIdx + i;
    const isCurrent = roundIdx === tRoundIndex && globalIdx === tMatchIndex;
    const isDone = match.winner !== null;
    const aWon = isDone && match.winner.titulo === match.a.titulo;
    const bWon = isDone && match.winner.titulo === match.b.titulo;

    html += `<div class="tSideMatch ${isCurrent ? 'tSideMatchActive' : ''} ${isDone ? 'tSideMatchDone' : ''}">
      <div class="tSideSlot ${aWon ? 'tSideWon' : ''} ${isDone && !aWon ? 'tSideLost' : ''}">
        <img class="tSideThumb" src="${match.a.imagen}" alt="${match.a.titulo}" title="${match.a.titulo}">
      </div>
      <div class="tSideDivider"></div>
      <div class="tSideSlot ${bWon ? 'tSideWon' : ''} ${isDone && !bWon ? 'tSideLost' : ''}">
        <img class="tSideThumb" src="${match.b.imagen}" alt="${match.b.titulo}" title="${match.b.titulo}">
      </div>
    </div>`;
  });
