export default async function handler(req, res) {
  ...
  if (req.method === 'GET' && req.query.popular) {
    try {
      // Use discover sorted by popularity — covers famous films across all rating ranges
      ...
      for (let batch = 0; batch < TOTAL_PAGES / BATCH_SIZE; batch++) {
        const pages = Array.from({length: BATCH_SIZE}, (_, i) => batch * BATCH_SIZE + i + 1);
        const results = await Promise.all(pages.map(p =>
          fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_KEY}&language=en-US&sort_by=popularity.desc&vote_count.gte=300&page=${p}`)
            .then(r => r.json())
        ));
        const batch_movies = results.flatMap(d => d.results || [])
          .filter(m => {
        ...
        allMovies = allMovies.concat(batch_movies);
      }
