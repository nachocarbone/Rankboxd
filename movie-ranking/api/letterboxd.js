export default async function handler(req, res) {
  ...
  if (req.method === 'GET' && req.query.popular) {
    try {
      // Fetch from multiple endpoints to get famous movies across the full rating spectrum
      const popularPages = Array.from({length: 20}, (_, i) => i + 1).map(p =>
        fetch(`https://api.themoviedb.org/3/movie/popular?api_key=${TMDB_KEY}&language=en-US&page=${p}`).then(r => r.json())
      );
      const topPages = Array.from({length: 5}, (_, i) => i + 1).map(p =>
        fetch(`https://api.themoviedb.org/3/movie/top_rated?api_key=${TMDB_KEY}&language=en-US&page=${p}`).then(r => r.json())
      );
      const nowPlayingPages = Array.from({length: 5}, (_, i) => i + 1).map(p =>
        fetch(`https://api.themoviedb.org/3/movie/now_playing?api_key=${TMDB_KEY}&language=en-US&page=${p}`).then(r => r.json())
      );

      const results = await Promise.all([...popularPages, ...topPages, ...nowPlayingPages]);

      // Deduplicate by id, require poster + meaningful vote count
      const seen = new Set();
      const movies = results.flatMap(d => d.results || [])
        .filter(m => {
          if (!m.poster_path || m.vote_count < 500) return false;
        ...
        .map(m => ({
          ...
          tmdbId: m.id,
          rating: m.vote_average ? Math.min(5, Math.round((m.vote_average / 2) * 10) / 10) : null,
        }))
  ...
  async function worker() {
    while (queue.length > 0) {
      ...
      try {
        ...
        // Strategy 3: try simplified title (remove subtitles after colon/dash)
        if (!result && titulo.includes(':')) {
          const simplified = encodeURIComponent(titulo.split(':')[0].trim());
          const url3 = `https://api.themoviedb.org/3/search/movie?api_key=${tmdbKey}&query=${simplified}&language=en-US`;
          const r3 = await fetch(url3);
          const data3 = await r3.json();
          if (data3.results) {
            const withPoster = data3.results.filter(m => m.poster_path);
            if (withPoster.length > 0) {
              if (year) {
                const yearMatch = withPoster.find(m => m.release_date && m.release_date.startsWith(year));
                result = yearMatch || withPoster[0];
              } else {
                result = withPoster[0];
              }
        ...
        if (result) {
          movies.push({
            ...
            tmdbId: result.id,
            rating: result.vote_average ? Math.min(5, Math.round((result.vote_average / 2) * 10) / 10) : null,
          });
        }
      } catch (e) { /* skip */ }
    }
