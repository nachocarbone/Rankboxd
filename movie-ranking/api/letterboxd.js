    ...
    try {
      ...
      const movies = results.flatMap(d => d.results || [])
        ...
        .map(m => ({
          ...
          tmdbId: m.id,
          rating: m.vote_average ? Math.min(5, Math.round((m.vote_average / 2 + 0.3) * 10) / 10) : null,
        }));
      ...
      try {
        ...
        if (result) {
          movies.push({
            ...
            tmdbId: result.id,
            rating: result.vote_average ? Math.min(5, Math.round((result.vote_average / 2 + 0.3) * 10) / 10) : null,
          });
        }
      } catch (e) { /* skip */ }
    }
